...
The coordinator routes requests automatically to the right specialist.

- General questions route to InfoAgent
- New image analysis routes to AnalysisPipeline
- Refinement requests route to DesignEditor

## Troubleshooting

### 1) Only venv appears in the app dropdown

Cause:
ADK is discovering directories as apps from your current launch location.

Fix:
- Start ADK from D:/AI_project/Feedback_Agent
- Use this command:

  python -m google.adk.cli web --host 127.0.0.1 --port 8000 .

- Refresh the browser and re-open the dropdown.

### 2) Port bind error on 8000
...
Fix:
- Stop the process on port 8000, then restart ADK.
- PowerShell example:
...
### 3) API key not detected

Fix checklist:
- Verify .env exists in the project root
- Verify key name is GOOGLE_API_KEY or GEMINI_API_KEY
- Restart ADK after editing .env

## Security

- Keep .env out of source control
- Rotate keys immediately if they were ever exposed
- Use least-privilege keys for development environments
...
Add your preferred license for distribution and collaboration.
